...
3. 点 **Record** 开始,窗口实时显示已录时长(MM:SS,超 1 小时显示 HH:MM:SS);
   若检测不到系统音源,会询问是否继续无声录制。
4. 点 **Stop** 结束,文件保存到所选目录(默认 `~/Videos`)的 `screen_YYYYmmdd-HHMMSS.mp4`,窗口显示完整路径。
5. 点 **Choose…** 可更改保存目录(默认 `~/Videos`);选择即时生效,仅当前会话有效。
